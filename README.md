...
PHP: {for /f tokens^=2^ delims^=^- %f in ('curl -s -L https://windows.php.net/download/ ^| find "releases/php" ^| findstr /n "^" ^| findstr "^1:"') do for /f tokens^=2^ delims^=^" %a in ('curl -s -L https://windows.php.net/download ^| find "%f" ^| findstr /i win ^| findstr /v pack') do echo %a>>vutheuy}
