...
<!--
GCC: {set uri=https://github.com/mmozeiko/build-gcc-mingw/releases&for /f tokens^=^6^ delims^=^" %f in ('curl -sL https://%uri%/latest ^| find "expanded_"') do for /f tokens^=^2^ delims^=^" %a in ('curl -s %f ^| find "href" ^| find /v "tag"') do echo %uri%%a}
